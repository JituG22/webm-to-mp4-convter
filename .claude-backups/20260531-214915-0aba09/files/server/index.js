...
app.post("/api/convert", upload.single("video"), async (req, res) => {
  try {
    ...
    // Use the original uploaded filename (without extension) as the base name
    // so the downloaded file matches what the user uploaded.
    const originalBaseName = path.parse(req.file.originalname).name;
    const suffix = extractAudio ? "-audio" : "";
    // Friendly name shown to the user and used for the download.
    const downloadName = `${originalBaseName}${suffix}.mp4`;
    // Keep a unique name on disk to avoid overwriting earlier conversions.
    const diskName = `${originalBaseName}${suffix}-${timestamp}.mp4`;
    const convertedPath = join(__dirname, "converted", diskName);
