...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    // Keep the original filename so uploads/conversions stay consistent.
    // A re-upload of the same name simply overwrites the previous file.
...
app.post("/api/convert", upload.single("video"), async (req, res) => {
  try {
    ...
    // Use the original uploaded filename (without extension) as the base name so
    // the converted file keeps the same name everywhere. Re-converting the same
    // file just overwrites the previous output.
    const originalBaseName = path.parse(req.file.originalname).name;
    const suffix = extractAudio ? "-audio" : "";
    const downloadName = `${originalBaseName}${suffix}.mp4`;
    const convertedPath = join(__dirname, "converted", downloadName);
